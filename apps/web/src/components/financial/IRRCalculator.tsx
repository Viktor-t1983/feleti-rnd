...
import { IRRInput } from '../../types/financial.types';
...
export const IRRCalculator: React.FC = () => {
  ...
  const handleCalculate = async (): Promise<void> => {
    ...
    try {
      ...
      const response = await financialApi.calculateIRR(input);
      setResult(response.irr);
      toast.success('Расчёт IRR выполнен успешно!');
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Произошла ошибка при расчете IRR';
    ...
    >
      ...
      <div className="space-y-6">
        ...
        {result !== null && (
          ...
          >
            ...
            <p className="mt-2 text-green-600 dark:text-green-400">
              {result > 0 ? (
                <>Внутренняя норма доходности составляет {result.toFixed(2)}%.</>
              ) : (
                <>IRR не может быть рассчитана для данного набора денежных потоков.</>
              )}
            </p>
          </div>
        )}
        ...
        <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-md">
          <h4 className="font-medium text-blue-800 dark:text-blue-200 mb-2">Что такое IRR?</h4>
          <p className="text-blue-700 dark:text-blue-300 text-sm">
            Внутренняя норма доходности (IRR) — это процентная ставка, при которой чистая
            приведенная стоимость инвестиционного проекта равна нулю. IRR используется для оценки
            привлекательности инвестиций: чем выше IRR, тем более привлекателен проект.
          </p>
        </div>
      </div>
    </div>
